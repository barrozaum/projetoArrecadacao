...
// atribuo o valor formatado na variavel valor
    var valor = preencheZeros(this.value, 5);
...
//        Requisição pelo Method POST
        ...
        url: "recursos/includes/validar/validarRua.php?cod=" + param,
//        dados passados
        data: {
            cod: param
        },
...
// cod = parametro enviado da linha (Codigo Rua, Bairrro)
    $(document).on('click', '#edit-editar', function (e) {
        ...
        $.post('recursos/includes/formulario/formularioTabelaRua.php',
                {id: 1,
    ...
    $(document).on('click', '#listar_ruas', function (e) {
        $("#div_button").html('<button type="button" id="btn_nao_listar">Esconder Ruas </button>');
        ...
        $('#listar').load('recursos/includes/listar/listarRuas.php');
    });

    $(document).on('click', '#btn_nao_listar', function (e) {
        $("#div_button").html('<button type="button" id="listar_ruas">Listar Ruas </button>');
        $('#listar').html('');
    });
