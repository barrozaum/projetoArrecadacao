...
//    aplica filtro na string enviada (LetraMaiuscula)
    $zona_Letra_Maiscula = letraMaiuscula($_POST['txt_alterar_zona']);
    $valor_Letra_Maiscula = letraMaiuscula($_POST['txt_alterar_valor']);
    $codigo_utilizacao_Letra_Maiscula = letraMaiuscula($_POST['txt_alterar_cod_utilizacao']);
    $codigo_categoria_Letra_Maiscula = letraMaiuscula($_POST['txt_alterar_cod_cat']);
...
// verifico se o tamanho do campo é correto
    ...
    if ((strlen($zona_Letra_Maiscula) > 0 && strlen($zona_Letra_Maiscula) < 11) || is_int($zona_Letra_Maiscula) === TRUE) {
        ...
        $array_erros['txt_alterar_zona'] = 'POR FAVOR ENTRE COM UMA ZONA VÁLIDA \n';
    }
...
//    filtro pra saber se o valor está correto
    if (is_numeric(inserirDinheiro($valor_Letra_Maiscula)) && strlen($valor_Letra_Maiscula) >= 3) {
        ...
        $array_erros['txt_alterar_valor'] = 'POR FAVOR ENTRE COM UM VALOR (UFIR) VÁLIDO \n';
    }
...
//    CODIGO UTILIZACAO
    if ((strlen($codigo_utilizacao_Letra_Maiscula) > 0 && strlen($codigo_utilizacao_Letra_Maiscula) < 11) || is_int($codigo_utilizacao_Letra_Maiscula) === TRUE) {
        ...
        $array_erros['txt_alterar_cod_utilizacao'] = 'POR FAVOR ENTRE COM UMA UTILIZAÇÃO VÁLIDA \n';
    }
...
//    CODIGO CATEGORIA
    if ((strlen($codigo_categoria_Letra_Maiscula) > 0 && strlen($codigo_categoria_Letra_Maiscula) < 11) || is_int($codigo_categoria_Letra_Maiscula) === TRUE) {
        ...
        $array_erros['txt_alterar_cod_cat'] = 'POR FAVOR ENTRE COM UMA CATEGORIA VÁLIDA \n';
    }
...
//      Comando sql a ser executado  
        $sql = "UPDATE Valor_M2_Construcao "
                . "SET Vlr_M2_Construcao = '$valor'"
...
//          Caso tenha errro 
//          lanço erro na tela
            die('<script>window.alert("Erro ao Alterar  !!!");location.href = "../../../TabelaValorM2Construcao.php";</script>'); /* É disparado em caso de erro na inserção de movimento */
        } else {
...
//        fecho conexao
        ...
        <script>
            window.alert("<?php echo "Valor Construção Alterada com Sucesso !!!"; ?> ");
