...
// verifico se o tamanho do campo é correto
    ...
    if ((strlen($codigo_Letra_Maiscula) > 0 && strlen($codigo_Letra_Maiscula) < 11) || is_int($codigo_Letra_Maiscula) === TRUE) {
        ...
        $array_erros['txt_excluir_codigo'] = 'POR FAVOR ENTRE COM UM CÓDIGO VÁLIDO !!! <BR />';
    }
...
// verifico se tem erro na validação
   if (empty($array_erros)) {
        try {
            ...
            $stmt = $pdo->prepare("DELETE FROM Bairro WHERE Cod_banco =:codigo");
    ...
    header("Location: ../../../TabelaBanco.php");
