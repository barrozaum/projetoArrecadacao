...
// Próximo valor de inscrição a ser cadastrado
function retornaProximoValor($pdo) {


// preparo para realizar o comando sql
    $sql = "SELECT Prox_Inscricao_Imob FROM sisparametros";
    $query = $pdo->prepare($sql);
//executo o comando sql
    $query->execute();
...
// Faço uma comparação para saber se a busca trouxe algum resultado
    if (($dados = $query->fetch()) == true) {
        $achou = $dados['Prox_Inscricao_Imob'];
    } else {
        $achou = $sql;
    }
    ...
    if ($numero_registro > 0) {
        ...
        for ($i = 0; $i < $numero_registro; $i++) {
            $linha[$i + 1] = Array(
                "Achou" => "1",
                "Ano" => $result[$i]['Ano'],
                "Valor" => mostrarDinheiro($result[$i]['Valor']),
